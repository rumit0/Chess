    ...
    class Moves
    {
        ...
        bool CanFigureMove()
        {
            switch (fm.figure)
            {
                ...
                case Figure.whitePanw:
                case Figure.blackPanw:
                        return CanPanwMove();
        ...
        bool CanPanwMove()
        {
            ...
            return CanPanwGo(stepY) || CanPanwJump(stepY) || CanPanwEat(stepY);
        }

        private bool CanPanwEat(int stepY)
        {
        ...
        private bool CanPanwJump(int stepY)
        {
            if (board.GetFigureAt(fm.to) == Figure.none)
                if (fm.DeltaX == 0)
        ...
        private bool CanPanwGo(int stepY)
        {
            if (board.GetFigureAt(fm.to) == Figure.none)
                if (fm.DeltaX == 0)
                    if (fm.DeltaY == stepY)
